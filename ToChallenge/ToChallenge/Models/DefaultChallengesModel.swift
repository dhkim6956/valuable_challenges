...
struct DefaultChallenge {
    ...
    let sort: UserChallenge.challengeSort                     //나만의 도전, 일반 도전, 서바이벌 도전
    let category: UserChallenge.challengeCategory             //자격증, 코딩, 운동, 외국어, 독서, 기타
    let description: String                     //설명
    let authenticationMethod: String            //인증방법
    let authenticationPeriod: UserChallenge.challengePeriod   //인증주기
    let duration: Int
...
var DefaultChallenges: [DefaultChallenge] = {
    ...
    challenges.append(DefaultChallenge(title: "Front-end 정복해보자", sort: .normal, category: .coding, description: "30일 동안 html, css, javascript에 대한 개념을 잡을 수 있는 과정", authenticationMethod: "Dream Coding 무료 동영상 강의를 듣고 해당 강의에서 작성한 코드를 캡쳐하여 인증", authenticationPeriod: .everyDay, duration: 30, challengeAdded: false))
    ...
    challenges.append(DefaultChallenge(title: "다양한 장르의 책을 한달에 1권씩 1년동안 읽기", sort: .normal, category: .reading, description: "문체부 추천도서 / 교과 연계도서를 읽고 인증하는 과정", authenticationMethod: "월 1회 독후감을 작성하거나 책의 핵심 내용을 요약하여 인증하는 과정", authenticationPeriod: .everyMonth, duration: 365, challengeAdded: false))
    ...
    challenges.append(DefaultChallenge(title: "매일 영어 일기 쓰기", sort: .normal, category: .language, description: "영어 writing 실력을 늘리기 위해 100일간 영어 일기(최소 한줄)를 쓰기를 도전해보자", authenticationMethod: "매일 영어 일기를 쓰고 쓴 내용을 사진으로 찍어 인증하기", authenticationPeriod: .everyDay, duration: 100, challengeAdded: false))
